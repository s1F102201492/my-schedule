...
const TodoList: React.FC<TodoListProps> = ({ todos }) => {
  ...
  const todaytodos:TodoProps[] = todos.filter(todo => {
    ...
    todayslash >= ChangeSlashDay(new Date(todo.startdate)) && todayslash <= ChangeSlashDay(new Date(todo.enddate));
  ...
  const notchecktodos = todaytodos.filter(todo => todo.checkedDates[todayslash] == false);
  const checkedtodos = todaytodos.filter(todo => todo.checkedDates[todayslash] == true);
